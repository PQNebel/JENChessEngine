use super::*;
#[inline(always)]
pub fn make_irreversible_move(game: &mut Game, cmove: &Move) {
    make_move(game, cmove, &mut SearchEnv::new())
}

#[inline(always)]
pub fn make_move(game: &mut Game, cmove: &Move, envir: &mut SearchEnv) {
    ...
    //Push to unmake_stack
    envir.push_to_unmake_stack(UnmakeFrame::new(game.castling_ability, game.enpassant_square, game.half_moves, captured));
...
pub fn unmake_move(game: &mut Game, cmove: &Move, envir: &mut SearchEnv) {
    let from_square = cmove.from_square();
    let to_square   = cmove.to_square();
    let promotion   = cmove.promotion();
    let piece       = cmove.piece();
    ...
    let unmake_frame = envir.pop_from_unmake_stack();

    //decrement fullmoves & switch back player
    if game.active_player == Color::Black {
        game.full_moves -= 1;
    }
    game.active_player = opposite_color(game.active_player);
    ...
    //Update castling abililties
    game.castling_ability = unmake_frame.castling_ability_before;

    //Reset enpassant
    game.enpassant_square = unmake_frame.enpassant_square_before;

    //Unmake promotion
    if promotion != Piece::None as u8 {
        //Spawn promoted
        game.bitboards[promotion as usize].unset_bit(to_square);
    }
    ...
    //Unmake castling
        ...
        match to_square {
            62 => { //White kingside
                game.bitboards[Piece::WhiteRook as usize].unset_bit_sq(Square::f1);
                game.bitboards[Piece::WhiteRook as usize].set_bit_sq(Square::h1);
                game.white_occupancies.unset_bit_sq(Square::f1);
                game.white_occupancies.set_bit_sq(Square::h1);
                game.all_occupancies.unset_bit_sq(Square::f1);
                game.all_occupancies.set_bit_sq(Square::h1);

                //Uppdate piece table
                game.piece_table[Square::f1 as usize] = Piece::None;
                game.piece_table[Square::h1 as usize] = Piece::WhiteRook;
            }
            58 => { //White queenside
                game.bitboards[Piece::WhiteRook as usize].unset_bit_sq(Square::d1);
                game.bitboards[Piece::WhiteRook as usize].set_bit_sq(Square::a1);
                game.white_occupancies.unset_bit_sq(Square::d1);
                game.white_occupancies.set_bit_sq(Square::a1);
                game.all_occupancies.unset_bit_sq(Square::d1);
                game.all_occupancies.set_bit_sq(Square::a1);

                //Uppdate piece table
                game.piece_table[Square::d1 as usize] = Piece::None;
                game.piece_table[Square::a1 as usize] = Piece::WhiteRook;
            }
            6 => { //Black kingside
                game.bitboards[Piece::BlackRook as usize].unset_bit_sq(Square::f8);
                game.bitboards[Piece::BlackRook as usize].set_bit_sq(Square::h8);
                game.black_occupancies.unset_bit_sq(Square::f8);
                game.black_occupancies.set_bit_sq(Square::h8);
                game.all_occupancies.unset_bit_sq(Square::f8);
                game.all_occupancies.set_bit_sq(Square::h8);

                //Uppdate piece table
                game.piece_table[Square::f8 as usize] = Piece::None;
                game.piece_table[Square::h8 as usize] = Piece::BlackRook;
            }
            2 => { //Black queenside
                game.bitboards[Piece::BlackRook as usize].unset_bit_sq(Square::d8);
                game.bitboards[Piece::BlackRook as usize].set_bit_sq(Square::a8);
                game.black_occupancies.unset_bit_sq(Square::d8);
                game.black_occupancies.set_bit_sq(Square::a8);
                game.all_occupancies.unset_bit_sq(Square::d8);
                game.all_occupancies.set_bit_sq(Square::a8);

                //Uppdate piece table
                game.piece_table[Square::d8 as usize] = Piece::None;
                game.piece_table[Square::a8 as usize] = Piece::BlackRook;
            }
            _ => unreachable!()
        }
    }
    ...
    //Increment half moves counter if quet and reset if pawn
    if piece == Piece::WhitePawn as u8 || piece == Piece::BlackPawn as u8 {
        game.half_moves = unmake_frame.half_moves_before;
    }
    else {
        game.half_moves -= 1;
    }
    ...
    //Update piece table
    game.piece_table[from_square as usize] = piece_from_u8(piece);
    game.piece_table[to_square as usize] = unmake_frame.captured;

    //Update bitboards
    game.bitboards[piece as usize].set_bit(from_square);
    game.bitboards[piece as usize].unset_bit(to_square);

    game.all_occupancies.set_bit(from_square);
    game.all_occupancies.unset_bit(to_square);

    if game.active_player == Color::White {
        game.white_occupancies.set_bit(from_square);
        game.white_occupancies.unset_bit(to_square);
    } else {
        game.black_occupancies.set_bit(from_square);
        game.black_occupancies.unset_bit(to_square);
    }
    ...
    if capturing {
        ...
        if enpassant {
            if game.active_player == Color::White {
                ...
                game.black_occupancies.set_bit(to_square + 8);
                game.all_occupancies.set_bit(to_square + 8);
                game.piece_table[(to_square + 8) as usize] = Piece::BlackPawn;
            }
            else {
                game.bitboards[Piece::WhitePawn as usize].set_bit(to_square - 8);
                game.white_occupancies.set_bit(to_square - 8);
                game.all_occupancies.set_bit(to_square - 8);
                game.piece_table[(to_square - 8) as usize] = Piece::WhitePawn;
            }
        } else {
            //Regular capture
            let captured = unmake_frame.captured;
...
mod make_tests {
    ...
    pub fn board_correct_after_move_with_rook() {
        ...
        make_irreversible_move(&mut game, &Move::new_friendly(Square::b7, Square::e7, Piece::WhiteRook, Piece::None, false, false, false, false));
    ...
    pub fn board_correct_after_enpassant_capture() {
        ...
        make_irreversible_move(&mut game, &Move::new_friendly(Square::e5, Square::f6, Piece::WhitePawn, Piece::None, true, false, true, false));
    ...
    pub fn switches_active_player_after_move() {
        ...
        make_irreversible_move(&mut game, &moves[0]);
    ...
    pub fn resets_half_moves_on_pawn_move() {
        ...
        make_irreversible_move(&mut game, &Move::new_friendly(Square::d2, Square::d3, Piece::WhitePawn, Piece::None, false, false, false, false));
    ...
    pub fn resets_half_moves_on_capture() {
        ...
        make_irreversible_move(&mut game, &Move::new_friendly(Square::f1, Square::b5, Piece::WhiteBishop, Piece::None, true, false, false, false));
        assert!(game.half_moves == 0);
    }
    ...
    pub fn increments_half_moves_on_quiet_move() {
        let mut game = Game::new_from_fen("rnbqkbnr/p1pppppp/8/1p6/8/4P3/PPPP1PPP/RNBQKBNR w KQkq - 8 1").unwrap();
        make_irreversible_move(&mut game, &Move::new_friendly(Square::f1, Square::c4, Piece::WhiteBishop, Piece::None, false, false, false, false));
    ...
    pub fn pawn_double_push_sets_enpassant_square() {
        let mut game = Game::new_from_start_pos();
        make_irreversible_move(&mut game, &Move::new_friendly(Square::d2, Square::d4, Piece::WhitePawn, Piece::None, false, true, false, false));
    ...
    pub fn enpassant_capture_kills_the_other_pawn() {
        ...
        make_irreversible_move(&mut game, &Move::new_friendly(Square::a5, Square::b6, Piece::WhitePawn, Piece::None, true, false, true, false));
    ...
    pub fn rooks_correctly_kingside() {
        ...
        make_irreversible_move(&mut game, &Move::new_friendly(Square::e1, Square::g1, Piece::WhiteKing, Piece::None, false, false, false, true));
    ...
    pub fn rooks_correctly_queenside() {
        let mut game = Game::new_from_fen("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 3 7").unwrap();
        make_irreversible_move(&mut game, &Move::new_friendly(Square::e1, Square::c1, Piece::WhiteKing, Piece::None, false, false, false, true));
        assert_eq!(game.get_piece_bitboard(Piece::WhiteRook).get_bit_sq(Square::e1), false);
    ...
    pub fn moving_rook_disables_castling_for_that_side() {
        ...
        make_irreversible_move(&mut game, &Move::new_friendly(Square::h1, Square::h2, Piece::WhiteRook, Piece::None, false, false, false, false));
        assert!(game.castling_ability & CastlingAbility::WhiteKingSide as u8 == 0);
        assert!(game.castling_ability & CastlingAbility::WhiteQueenSide as u8 != 0);
    }
    ...
    pub fn capturing_rook_disables_castling_for_that_side() {
        ...
        make_irreversible_move(&mut game, &Move::new_friendly(Square::b3, Square::a1, Piece::BlackKnight, Piece::None, true, false, false, false));
        assert!(game.castling_ability & CastlingAbility::WhiteKingSide as u8 != 0);
        assert!(game.castling_ability & CastlingAbility::WhiteQueenSide as u8 == 0);
    }
    ...
    pub fn white_pawn_promotes_correctly_on_quietly_reaching_back_row() {
        ...
        make_irreversible_move(&mut game, &Move::new_friendly(Square::b7, Square::b8, Piece::WhitePawn, Piece::WhiteQueen, false, false, false, false));
    ...
    pub fn black_pawn_promotes_correctly_on_quietly_reaching_back_row() {
        ...
        make_irreversible_move(&mut game, &Move::new_friendly(Square::d2, Square::d1, Piece::BlackPawn, Piece::BlackRook, false, false, false, false));
    ...
    pub fn pawn_promotes_correctly_on_back_row_capture() {
        ...
        make_irreversible_move(&mut game, &Move::new_friendly(Square::b7, Square::c8, Piece::WhitePawn, Piece::WhiteBishop, false, false, false, false));
    ...
    pub fn moving_the_king_disables_castling() {
        let mut game = Game::new_from_fen("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 3 7").unwrap();
        make_irreversible_move(&mut game, &Move::new_friendly(Square::e1, Square::f1, Piece::WhiteKing, Piece::None, false, false, false, false));
        assert!(game.castling_ability & CastlingAbility::WhiteKingSide as u8 == 0);
        assert!(game.castling_ability & CastlingAbility::WhiteQueenSide as u8 == 0);
    }
    ...
    pub fn capturing_bishop_results_in_correct_bitboards() {
        ...
        make_irreversible_move(&mut game, &Move::new_friendly(Square::g8, Square::h6, Piece::BlackPawn, Piece::None, true, false, false, false));
